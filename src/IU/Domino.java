...
public class Domino {
	...
	private static int numJugadores; // valor de 2,3 o 4
	 ...
	 *
	 * @return boolean, se pregunta si se quiere seguir jugando, devuelve true si SI, false si NO.
	 */
	private static boolean iniciarPartida() {
		...
		do {
			...
			if (identificadorJugador == numJugadores) {
			...
			&& acabarPartidaPorFaltaDeOpciones(tablero, jugador, monton) == false);
	...
	public static void pedirNumJugadores() {
		int num;
	...
	private static void insertarJugador() {
		int num = 0;
		String nombre = "";
		String[] todosNombres = new String[numJugadores]; // Evitar nombres repetidos
	 ...
	 * menor puntuación tenga.
	 */
	private static void calcularPuntuacion() {
		int minimaPuntuacion = Integer.MAX_VALUE;
		String ganadorActual = "";
		String jugadorLlevaMano = "";
		int longitudNombreMasLargo = 0;

		for (int j = 0; j < jugador.length; j++) {
			// Coger el jugador con la puntuación mínima actual y al ganador actual.
			...
			// Coger el nombre que lleva la mano.
		...
		for (int n = "Nombre".length(); n < longitudNombreMasLargo; n++) {
			System.out.print(" "); // Pintar espacio para que quede bien.
		}
	 ...
	 * indice del 'paquete' array String[] será la opción de la jugada a elegir. El 'paquete' con
	 * las posibles jugadas se pasará a otra función que leerá y ejecutará la opción. - Si se puede
	 ...
	 * pueden añadir fichas al tablero: 1º: El valor de este campo será siempre -1. 2º: Indica:
	 * pasar turno, ó coger ficha del montón, ó finalizar partida.
	 *
	 * @param tablero
	 * @param jugador
	 * @param monton
	...
	private static String[] posiblesJugadas(Tablero tablero, Jugador jugador, Monton monton) {
		...
		// [id]* == indiceFicha que está en la mano.
		//      [INDICE]	:              VALOR
		...
		String[] paquetes = new String[10]; // Mejor pasarse...
		String campo1 = "";
		String campo2 = "";
		...
		if (tablero.getNumFichasTablero() == 0) {
			...
			Deque<Ficha> fichasTablero = tablero.getDequeFichasTablero();
			...
			for (int indice = 0; indice < jugador.getLongitudFichasMano(); indice++) {
				...
				if ((fichasTablero.getFirst().getNumIzq() == jugador.getFichaMano(indice).getNumDer())
					|| (fichasTablero.getFirst().getNumIzq() == jugador.getFichaMano(indice).getNumIzq())) {
					colocarInicio = true;
				...
				if ((fichasTablero.getLast().getNumDer() == jugador.getFichaMano(indice).getNumDer())
					|| (fichasTablero.getLast().getNumDer() == jugador.getFichaMano(indice).getNumIzq())) {
					colocarFin = true;
	...
	private static int accionarJugada(Tablero tablero, Jugador jugador, Monton monton, String[] paquetes) {
		...
		System.out.print("Posibles opciones: ");
		...
		do {
			...
			// Se coge del monton y se retorna valor 55.
	...
	private static boolean acabarPartidaPorFaltaDeOpciones(Tablero tablero, Jugador[] jugador, Monton monton) {
		...
		for (int j = 0; j < jugador.length; j++) {
			...
			System.out.println(paquete.length + "-------------");
